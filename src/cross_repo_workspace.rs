//! Isolated multi-repository engineering workspaces.
//!
//! P5.1 deliberately keeps network resolution and arbitrary command execution
//! outside the core abstraction. Callers provide local Git checkouts, each is
//! verified against an immutable revision from [`CompatibilitySet`], and only
//! tracked files from that exact revision are materialized into isolated roots.
...
/// One local checkout offered as the source for a compatibility-set entry.
...
impl LocalRepositorySource {
    pub fn new(
        ...
        let validated = RepositoryRevision::new(&repository, &revision, &role)
...
/// A temporary Cargo `[patch]` override generated outside tracked manifests.
...
impl CargoPatchOverride {
    pub fn new(
        ...
        let provider_path = normalize_relative(&provider_path.into())?;
        Ok(Self {
            consumer_role,
            source,
            package,
...
/// Storage and fan-out limits for a cross-repository evaluation workspace.
    ...
    pub fn new(
        ...
        if max_repositories == 0 {
            return Err(CrossRepoWorkspaceError::InvalidPolicy(
                "max_repositories must be greater than zero".into(),
            ));
        }
...
/// Immutable evidence for one effective repository state.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Deterministic logical record of an effective dependency override.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Complete reproducibility record emitted by a materialized workspace.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Isolated roots for one exact compatibility set.
///
/// The abstraction performs only fixed local Git inspection (`rev-parse`,
/// `diff`, `ls-files`). It never fetches, checks out remote refs, or executes
...
impl CrossRepoWorkspace {
    pub fn materialize(
        ...
        if source_by_key.len() != compatibility.revisions().len() {
            return Err(CrossRepoWorkspaceError::SourceSetMismatch(
                "source contains an entry not present in the compatibility set".into(),
            ));
        }

        let allowed_patch_roles: BTreeSet<String> = allowed_patch_roles.into_iter().collect();
        ...
        for rule in &overrides {
            if !roles.contains(&rule.consumer_role) {
                return Err(CrossRepoWorkspaceError::UnknownRole(
                    rule.consumer_role.clone(),
                ));
            }
            if !roles.contains(&rule.provider_role) {
                return Err(CrossRepoWorkspaceError::UnknownRole(
                    rule.provider_role.clone(),
                ));
            }
        }
        ...
        let build = (|| {
            ...
            for (index, revision) in compatibility.revisions().iter().enumerate() {
                let key = (revision.repository.clone(), revision.role.clone());
                let source = source_by_key.get(&key).expect("validated source set");
    ...
    /// Return the generated Cargo config for a consumer role, if it has
    /// temporary overrides. The config lives outside every tracked repository
    /// root and can be supplied to Cargo with `--config <path>` by P5.2.
    ...
    /// Apply a PatchSet only to a role explicitly authorized by the task.
    pub fn apply_patch(
        &mut self,
        role: &str,
        patch_set: &PatchSet,
        ...
        let old_bytes = repo.state.usage().bytes;
        let child = repo
            .state
            .child(patch_set)
            .map_err(CrossRepoWorkspaceError::CandidateState)?;
        ...
        {
            let repo = self.repositories.get_mut(role).expect("role checked above");
            repo.state = child;
            repo.patch_set_ids.push(patch_set_id.clone());
        }
    ...
    fn refresh_override_configs(&mut self) -> Result<(), CrossRepoWorkspaceError> {
        ...
        for (index, (consumer, sources)) in grouped.iter().enumerate() {
            ...
            let path = next.join(format!("{index}.toml"));
...
impl fmt::Display for CrossRepoWorkspaceError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::RepositoryLimitExceeded { actual, limit } => {
                write!(f, "repository limit exceeded: {actual} > {limit}")
            }
            Self::TotalByteLimitExceeded { actual, limit } => {
                write!(f, "workspace byte limit exceeded: {actual} > {limit}")
            }
            ...
            Self::RevisionMismatch {
                ...
                f,
                "revision mismatch for role {role}: expected {expected}, got {actual}"
            ),
            ...
            Self::UnsupportedTrackedEntry(path) => {
                write!(f, "unsupported tracked filesystem entry: {path}")
            }
            Self::PatchRoleNotAllowed(role) => {
                write!(f, "task does not allow patches to repository role {role}")
            }
            Self::DuplicateOverride {
                ...
                f,
                "duplicate Cargo override for {consumer_role}: {source} / {package}"
            ),
...
fn verify_git_source(
    source: &LocalRepositorySource,
    expected: &RepositoryRevision,
) -> Result<(), CrossRepoWorkspaceError> {
    if !source.root.is_dir() {
        return Err(CrossRepoWorkspaceError::Git(format!(
            "{} is not a directory",
            source.root.display()
        )));
    }
...
fn git_output(root: &Path, args: &[&str]) -> Result<String, CrossRepoWorkspaceError> {
    ...
    String::from_utf8(output.stdout)
        .map_err(|_| CrossRepoWorkspaceError::Git("git output is not UTF-8".into()))
}
...
fn git_success(root: &Path, args: &[&str]) -> Result<(), CrossRepoWorkspaceError> {
    ...
    if status.success() {
        ...
        Err(CrossRepoWorkspaceError::Git(format!(
    ...
    for component in parsed.components() {
        match component {
            Component::Normal(value) => parts.push(
                value
                    .to_str()
                    .ok_or_else(|| CrossRepoWorkspaceError::UnsafeTrackedPath(path.into()))?,
            ),
...
mod tests {
    ...
    fn repository(name: &str, cargo_package: &str) -> (PathBuf, String) {
    ...
    fn policy() -> CrossRepoWorkspacePolicy {
        CrossRepoWorkspacePolicy::new(
            4,
            1_000_000,
            CandidateStoragePolicy::new(100, 500_000).unwrap(),
        )
        .unwrap()
    }

    #[test]
    fn materializes_exact_clean_revisions_and_ignores_untracked_files() {
        let (rsi_root, rsi_rev) = repository("rsi", "rsi-fixture");
        let (sci_root, sci_rev) = repository("scirust", "scirust-fixture");
        std::fs::write(sci_root.join("ignored-local.bin"), b"not part of revision").unwrap();
        let set = CompatibilitySet::new(
            vec![
                RepositoryRevision::new("Memorithm/RSI", &rsi_rev, "rsi").unwrap(),
                RepositoryRevision::new("Memorithm/scirust", &sci_rev, "scirust").unwrap(),
            ],
            "rustc stable",
            vec![],
        )
        ...
        let workspace = CrossRepoWorkspace::materialize(
            set,
            vec![
                LocalRepositorySource::new("Memorithm/RSI", "rsi", &rsi_rev, &rsi_root).unwrap(),
                LocalRepositorySource::new("Memorithm/scirust", "scirust", &sci_rev, &sci_root)
                    .unwrap(),
            ],
            vec!["rsi".into()],
            vec![],
        ...
        assert!(!workspace
            .root_for_role("scirust")
            .unwrap()
            .join("ignored-local.bin")
            .exists());
        let _ = std::fs::remove_dir_all(rsi_root);
        let _ = std::fs::remove_dir_all(sci_root);
    }
    ...
    fn rejects_revision_mismatch_and_dirty_tracked_sources() {
        ...
        let mismatch = CrossRepoWorkspace::materialize(
            set.clone(),
            vec![LocalRepositorySource::new("Memorithm/RSI", "rsi", &wrong, &root).unwrap()],
            vec![],
            vec![],
            policy(),
        )
    ...
    #[test]
    fn patch_allowlist_is_role_scoped_and_result_records_effective_state() {
        let (root, revision) = repository("rsi", "rsi-fixture");
        let set = CompatibilitySet::new(
            vec![RepositoryRevision::new("Memorithm/RSI", &revision, "rsi").unwrap()],
            "rustc stable",
        ...
        let mut denied = CrossRepoWorkspace::materialize(
            set.clone(),
            vec![source.clone()],
            vec![],
            vec![],
        ...
        .unwrap();
        assert!(matches!(
            denied.apply_patch("rsi", &patch),
            Err(CrossRepoWorkspaceError::PatchRoleNotAllowed(_))
        ));

        let mut allowed = CrossRepoWorkspace::materialize(
            set,
            vec![source],
            vec!["rsi".into()],
            vec![],
    ...
    #[test]
    fn cargo_override_is_external_to_repositories_and_recorded() {
